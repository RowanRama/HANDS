...
#         tendon_id: int
#             ID of the tendon in the system. This is used to identify which tendon is being referenced in tension_func_array.
#         tension_func_array: numpy.ndarray
#             1D (dim) numpy array. Contains the tension values for each of the tendons in the system.
#         n_elements: int
...
#     def __init__(self, vertebra_height, num_vertebrae, first_vertebra_node, final_vertebra_node, vertebra_mass, tendon_id, tension_func_array, vertebra_height_orientation, n_elements):
#         """
...
#         self.cached_tension = tension_func_array[tendon_id]
...
#         self.tensions = (
#             tension_func_array
#             if hasattr(tension_func_array, "__call__")
#             else lambda time_v: tension_func_array
#         )
...
#         self.initial_force_flag = 1
#         # Updating the cached tension value
#         self.cached_tension = latest_tensions[self.tendon_id]
...
#         self.force_data = self.compute_forces(self.cached_tension, np.array(self.vertebra_nodes), unit_norm_vector_array)
...
class TendonForces(NoForces):
    def __init__(
        ...
        self.vertebra_height_vector = vertebra_height_orientation * vertebra_height
        self.tendon_id = tendon_id
        self.tension_func_array = tension_func_array  # Now we store the full array
        self.n_elements = n_elements
        self.vertebra_weight_vector = np.array([0.0, 0.0, -vertebra_mass * 9.80665])
    ...
    def apply_forces(self, system: SystemType, time: np.float64 = 0.0):
        self.cached_tension = self.tension_func_array[self.tendon_id]
        ...
        self.force_data = self.compute_forces(
            self.cached_tension,
            np.array(self.vertebra_nodes),
            unit_norm_vector_array
        )
    ...
    def get_rotations(position_collection, director_collection, vertebra_nodes, vertebra_height_vector):
        ...
        for i in range(len(vertebra_nodes) + 1):
            ...
            x_current, y_current, z_current = position_collection[:, current_vertebra]
            x_next, y_next, z_next = position_collection[:, next_vertebra]
            ...
            delta_vector = (
                next_node + np.ascontiguousarray(next_rotation_matrix.T) @ np.ascontiguousarray(vertebra_height_vector)
            ) - (
                current_node + np.ascontiguousarray(current_rotation_matrix.T) @ np.ascontiguousarray(vertebra_height_vector)
            )
            ...
            unit_norm_vector_array[i - 1 if i > 0 else 0] = unit_norm_delta_vector
    ...
    def compute_torques(vertebra_height_vector, vertebra_nodes, transformed_force_data, n_elements, external_torques):
        torque_data = np.zeros((len(vertebra_nodes), 3), dtype=np.float64)
        for i in range(len(vertebra_nodes)):
            torque_data[i] = np.cross(vertebra_height_vector, transformed_force_data[i])
