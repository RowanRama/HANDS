...
class HierarchicalFingerEnv(gymnasium.Env):
    def __init__(self, base_env, reward_func):
        super().__init__()
        self.base_env = base_env
        self.reward_func = reward_func

        # Forward observation and action spaces from base env
        self.observation_space = self.base_env.observation_space
        self.action_space = self.base_env.action_space

        # Track previous values (used for rotation-based rewards)
        self.prev_angle = 0.0
        self.target_angle = np.pi / 2  # default 90° rotation

    def reset(self, **kwargs):
        self.prev_angle = 0.0
        self.target_angle = np.random.uniform(0, 2 * np.pi)  # Optional: random goal

        obs = self.base_env.reset(**kwargs)
        return obs

    def step(self, action):
        obs, _, done, truncated, info = self.base_env.step(action)

        # Compute custom reward using updated info and wrapper context
        reward = self.reward_func(info, self)
