...
def create_gif(outputs, gif_path="backbone_animation.gif"):
    ...
    def update(frame):
        ...
        dot_product = np.dot(np.reshape(cyl_dir, (3,)), z_axis)
        if abs(dot_product) > 1e-6:  # Tolerance for perpendicularity
            print(f"Warning: Cylinder direction is not perpendicular to the z-axis at frame {frame}.")
        ...
        # Update the cylinder in the 3D view
        if cylinder_surface is not None:
        ...
        cylinder_surface = ax1.plot_surface(x_grid_rotated, y_grid_rotated, z_grid_rotated, color='b', alpha=0.3)

        # Add circular disks at the upper and lower ends of the cylinder
        disk_z = np.array([-cylinder_length / 2, cylinder_length / 2])  # Lower and upper ends
        for z_offset in disk_z:
            disk_x = cylinder_radius * np.cos(theta)
            disk_y = cylinder_radius * np.sin(theta)
            disk_points = np.dot(rotation_matrix, np.array([disk_x, disk_y, np.full_like(disk_x, z_offset)]))
            disk_x_rotated = disk_points[0, :] + cyl_center[0]
            disk_y_rotated = disk_points[1, :] + cyl_center[1]
            disk_z_rotated = disk_points[2, :] + cyl_center[2]
            ax1.plot_surface(
                disk_x_rotated.reshape(1, -1),
                disk_y_rotated.reshape(1, -1),
                disk_z_rotated.reshape(1, -1),
                color='b',
                alpha=0.5,
            )
        ...
        # Calculate the projected rectangle in the XY plane
        cyl_dir_xy = np.reshape(cyl_dir[:2], (2,))  # Extract the XY components of the cylinder direction
        if np.linalg.norm(cyl_dir_xy) > 1e-6:  # Avoid division by zero
            cyl_dir_xy = cyl_dir_xy / np.linalg.norm(cyl_dir_xy)  # Normalize direction in XY plane
        else:
            cyl_dir_xy = np.array([1, 0])  # Default to x-axis if direction is zero

        perpendicular_dir = np.array([-cyl_dir_xy[1], cyl_dir_xy[0]])  # Perpendicular direction in XY plane

        # Calculate the rectangle corners
        half_length = cylinder_length / 2
        half_width = cylinder_radius
        corner1 = cyl_center[:2] + half_length * cyl_dir_xy + half_width * perpendicular_dir
        corner2 = cyl_center[:2] + half_length * cyl_dir_xy - half_width * perpendicular_dir
        corner3 = cyl_center[:2] - half_length * cyl_dir_xy - half_width * perpendicular_dir
        corner4 = cyl_center[:2] - half_length * cyl_dir_xy + half_width * perpendicular_dir

        # Plot the rectangle in the 2D view
        rectangle_x = [corner1[0], corner2[0], corner3[0], corner4[0], corner1[0]]
        rectangle_y = [corner1[1], corner2[1], corner3[1], corner4[1], corner1[1]]
        rectangle_patch, = ax2.plot(rectangle_x, rectangle_y, color='b', alpha=0.7)
