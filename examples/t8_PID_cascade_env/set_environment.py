...
class PID_Cascader: 
    def __init__(self, Kp = 5.0, Ki = 50.0, Kd = 0.0, num_action = 4, max_tension= None):
        """
        Initialize the PID cascader and pass the action values to tension values.
        
        number of tendons should be double of the number of actions
        Parameters:
        - Kp: Proportional gain
        - Ki: Integral gain
        - Kd: Derivative gain
        - max_tension: Tuple (min, max) to limit the output value
        """
        ...
        # Internal variables
        self._prev_error = np.zeros(num_action)
        self._integral = np.zeros(num_action)
        self._last_time = None
    def reset(self):
        """
        Reset the PID controller to its initial state.
        """
        self._prev_error = np.zeros(self.num_action)
        self._integral = np.zeros(self.num_action)
        self._last_time = None
    def compute(self, action, current_time):
        """
        Compute the PID output based on the error.
        
        Parameters:
        - action: The difference between the desired and current value
        - current_time: The current time (used for calculating the derivative term)
        
        Returns:
        - PID output
        """
        # Check if the error is a numpy array of correct size
        if not isinstance(action, np.ndarray) or action.shape != (self.num_action,):
            raise ValueError(f"Error must be a numpy array of shape ({self.num_action},)")
        ...
        # Integral term
        self._integral += action * dt
        I = self.Ki * self._integral

        # Derivative term
        D = 0
        if self.Kd > 0 and self._last_time is not None:
            if dt > 0:
                D = self.Kd * (action - self._prev_error) / dt
        ...
        if self.max_tension is not None:
            output = np.clip(output, -self.max_tension, self.max_tension)

        # Update internal state
        self._prev_error = action
        self._last_time = current_time

        # Initialize tensions to zero
        tensions = np.zeros(2 * self.num_action)

        # Update tensions based on output
        for i in range(self.num_action):
            if output[i] > 0:
                tensions[i] = output[i]
            else:
                tensions[self.num_action + i] = -output[i]
...
class Environment(gymnasium.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Action space is modified and restricted by the PID cascader
        self.action_space = spaces.Box(
            low=-10.0,
            high=10.0,
            shape=(len(self.directions)//2,),
            dtype=np.float64
        )
        ...
        self.pid_cascade = PID_Cascader(num_action=len(self.directions), max_tension=self.max_tension)

    def reset(self):
        """Reset the environment to initial state."""
        # Create simulator
        self.simulator = SoftRobotSimulator()
        self.pid_cascade.reset()
        n_elem = self.n_elem
        start = np.zeros((3,))
    ...
    def step(self, action):
        ...
        #
        self.action = action
        tension_value = self.pid_cascade.compute(action, self.time_tracker)
        
        # Update tendon forces
        # if the tension value is negative, clip to 0
        self.tensions[:] = tension_value
