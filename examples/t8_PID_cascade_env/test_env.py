...
def action_values(t, state):
    point_to_go = points_to_go[int(t//2.0) % len(points_to_go)]
    # Controller v1.1 for 4 tendons - Closed loop using PI controller
    current_x = state[10]
    current_y = state[21]
    # Compute errors
    error_x = point_to_go[0] - current_x
    error_y = point_to_go[1] - current_y
    return np.array([error_x, 0, error_y, 0])
...
def test_environment():
    ...
    "final_vertebra_node": [n_elem-2, n_elem//2-1, n_elem-2, n_elem//2-1, n_elem-2, n_elem//2-1, n_elem-2, n_elem//2-1],
    } # There are 8 tendons, they are placed in a circle around the rod. Four for full length and four for half length.

    env = Environment(n_elem=n_elem, mode=1, final_time= 20, target_position=np.array([0.5, 0.5, 0.5]), gravity_enable=False, tendon_config=tendon_config)
    ...
    for step in tqdm(range(num_steps)):
        ...
        action = action_values(step * dT_L, state)  # Use the tension function to get the action for this step
