...
#assume an environment setup with a low level controller
#write a modular RL script
#assume u can get an action space and a reward
#script to make an RL policy to 
#write elaborate points and merge  
...
def make_env():
    return Environment(
        ...
        max_tension=7.0 #not generalizable to diff max tensions
    )

def evaluate(model_path, log_dir, num_episodes=5):
    env = make_env()
    model = SAC.load(model_path, env=env)
    print("Action space for current env:", env.action_space)

    dT_L = env.time_step * env.num_steps_per_update

    all_outputs = []
    #mode 2 gives a random target every reset
    ...
    with open(os.path.join(log_dir, "eval_policy.pkl"), "wb") as f:
        pickle.dump(all_outputs, f)

    print(f"\n✅ Saved {num_episodes} evaluation episodes to eval_policy.pkl")

def plot_tension():
    with open("./sac/outputs.pkl", "rb") as f:
