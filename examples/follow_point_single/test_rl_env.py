...
def test_environment():
    ...
    env = Environment(n_elem=100, mode=1, final_time= 2, target_position=np.array([0.5, 0.5, 0.5]), gravity_enable=False)
    ...
    for step in tqdm(range(num_steps)):
        ...
        #+X, only moves in +X
        action = np.array([2.0, 0.0, 0.0, 6.0])   # Use the tension function to get the action for this step
