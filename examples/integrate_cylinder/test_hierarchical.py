...
def compute_reward(info, env, target_angle=None):
    ...
    if "cylinder_director" in info:
        ...
        director = info["cylinder_director"][..., 0]  # Get first point's director

        current_x = director[:, 0]  # Normal vector (z world, x cylinder)
        #project to XY (world) which is YZ (cylinder)
        current_angle = np.arctan2(current_x[2], current_x[1])  # Project to YZ plane

        print(f"Angle about Z in rad: {(current_angle)}")
        print(f"current_angle: {np.degrees(current_angle):.2f}°")
        
        env.prev_angle = current_angle #set for next timestep
...
def step_time():
    ...
    #print(outputs)
    return outputs 
# Create the env and pass in reward logic
# env = HierarchicalFingerEnv(
#     base_env=MultipleFinger(cylinder_enabled=True),
#     reward_func=lambda info: compute_reward(info, env),
...
#     print("cylin position:", info["cylinder_position"][..., 0]) #3 x 3 x 1 matrix
#     print("cylin director:", info["cylinder_director"])
#     print("Reward:", reward)
#     if done:
#         break
